...
def build_state_with_threshold(lm, obs, infos, prev_obs, prev_acts, state_length_threshold):
    """
    Build the state with threshold, if the state is too long, simplify it!
    """
    assert (prev_obs is not None) and (prev_acts is not None)
    states = []
    for prev_ob, ob, info, act in zip(prev_obs, obs, infos, prev_acts):
        sent = "[CLS] %s [SEP] %s [SEP] %s [SEP]" % (prev_ob, act, ob + info['inv'] + info['look'])
        ...
        states.append(State(lm.sent2ids(ob), lm.act2ids(info['look']), lm.act2ids(info['inv']), encoded_state))
    return states
