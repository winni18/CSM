...
class GPT2LM(BaseLM):
    ...
    def generate(self, input, k, mask_out=ILLEGAL_ACTIONS + NO_EFFECT_ACTIONS, key=None):
        ...
        # As we will fine-tune the model, we disable the directly mapping
        ...
        outputs, scores = generate_topk(self.model, input_ids=input_ids, do_sample=False, num_beams=min(k * 2, 40),
        ...
        self.generate_score_dict[key] = scores
        return actions, scores


    def generate_warmup(self, input, k, mask_out=ILLEGAL_ACTIONS + NO_EFFECT_ACTIONS, key=None):
        """
        This is only used in the first 20k steps
        """
        input_ids = self.sent2ids(input) if isinstance(input, str) else input
        if key is None:
            key = hash((tuple(input_ids), k))
        if key in self.generate_dict:
            return self.generate_dict[key], self.generate_score_dict[key]
        input_len = len(input_ids)
        input_ids = torch.tensor([input_ids]).to(device)
        mask_out = [self.tokenizer.encode(' ' + w)[0] for w in mask_out]
        outputs, scores = generate_topk(self.model, input_ids=input_ids, do_sample=False, num_beams=min(k * 2, 40),
...
def generate_topk(
    ...
    if pad_token_id is None and eos_token_ids is not None:
        # logger.warning("Setting `pad_token_id` to {} (first `eos_token_id`) to generate sequence".format(eos_token_ids[0]))
    ...
    if num_beams > 1:
        output, scores = _generate_beam_search_topk(
            self,
            input_ids,
            cur_len,
            max_length,
        ...
        raise Exception("Does not allow _generate_no_beam_search()!")
        output, scores = _generate_no_beam_search(
            self,
            input_ids,
            cur_len,
            max_length,
